...
- 公开订阅 / 退订（退订链接带 token）
- 订阅前**Webhook 即时自检**：填完机器人地址点「测试 Webhook」，立即发一条测试消息验证地址有效、能收到推送（降低填错/失效地址导致收不到推送的失败率）；自检接口同样带每 IP 限流（20 次/时）
...
└── functions/
    ...
    └── api/
        ...
        ├── check.js          # Webhook 即时自检（订阅前验证地址有效，发测试消息）
...
2. 切到「订阅」：填你的机器人 Webhook（企微/飞书/钉钉群 → 添加群机器人 → 复制地址）→ **可先点「测试 Webhook」发一条测试消息确认能收到**，再勾选源与频道、可选填关键词 → 订阅。
