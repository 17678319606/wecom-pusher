...
export async function onRequestGet() {
  ...
  // 周期为每 2 小时，超过 150 分钟（1.25 个周期 + 余量）未触发即判定 cron 失效
  const cronAlive = last > 0 && staleMs < 150 * 60 * 1000;
  return json({
    ok: true,
    cronAlive,
    ...
    note: cronAlive
      ? '定时任务正常：cron 在按时触发 /api/cron/tick（每 2 小时一次）。'
      : '定时任务未触发或已失效。EdgeOne Makers(direct-upload) 不会自动应用 edgeone.json 的 schedules，请二选一：(1) 控制台「项目 → 定时任务」手动建 /api/cron/tick (cron=0 */2 * * *, POST, Asia/Shanghai)；(2) 用 cron-job.org 等外部服务每 2 小时 POST 一次该地址（端点幂等、哈希去重）。',
  });
}
